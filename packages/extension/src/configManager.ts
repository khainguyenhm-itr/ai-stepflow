...
export class ConfigManager {
  ...
  public async ensureGlobalClaudeMd(): Promise<void> {
    ...
    const block = [
      ...
      'When creating a workflow or dispatching subagents for a multi-phase task, **prefer `superpowers:*` skills as the process for each phase** — they encode a shared, verified standard, so they win over ad-hoc prompts or other skills whenever one applies. Map each phase to its superpowers skill:',
      ...
      'Only fall back to a non-superpowers skill or a custom prompt when no `superpowers:*` skill covers the phase, and state briefly why.',
